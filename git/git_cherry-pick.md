...
* git cherry-pick --ff ..next 
    履歴が線形で、HEAD が next の祖先である場合、作業ツリーを更新し、HEAD ポインタを next に一致するように進めます。 それ以外の場合は、HEAD ではなく next にあるコミットによって導入された変更を現在のブランチに適用し、新しい変更ごとに新しいコミットを作成します。
* git rev-list --reverse master -- README | git cherry-pick -n --stdin
    README に触れたマスター ブランチのすべてのコミットによって導入された変更を作業ツリーとインデックスに適用します。これにより、結果を検査して、適切な場合は単一の新しいコミットにすることができます。