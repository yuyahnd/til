...
* --progress  
このオプションは、addコマンドとupdateコマンドでのみ有効です。-qオプションが指定されていない限り、端末に接続されている場合、進捗状況はデフォルトで標準エラーストリームに出力されます。このフラグを指定すると、標準エラーストリームが端末に出力されていない場合でも、進捗状況が強制的に出力されます。
...
* -f
* --force  
このオプションは、add、deinit、update コマンドでのみ有効です。add 実行時、通常は無視されるサブモジュールパスの追加を許可します。deinit 実行時、サブモジュールの作業ツリーは、ローカルの変更が含まれていても削除されます。update 実行時（チェックアウト手順でのみ有効）は、別のコミットに切り替える際にサブモジュール内のローカルの変更を破棄します。また、サブモジュールを含むリポジトリのインデックスにリストされているコミットが、サブモジュールでチェックアウトされたコミットと一致する場合でも、常にサブモジュール内でチェックアウト操作を実行します。

* --cached  
このオプションはステータスコマンドとサマリーコマンドでのみ有効です。これらのコマンドは通常、サブモジュールHEAD内のコミットを使用しますが、このオプションを指定すると、代わりにインデックスに保存されているコミットが使用されます。
...
* -n  
* --summary-limit  
このオプションは summary コマンドでのみ有効です。サマリーのサイズ（表示されるコミットの総数）を制限します。0 を指定するとサマリーは無効になり、負数を指定すると無制限になります（デフォルト）。この制限は変更されたサブモジュールにのみ適用されます。追加/削除/タイプ変更されたサブモジュールの場合、サイズは常に 1 に制限されます。

* --remote  
このオプションは update コマンドでのみ有効です。サブモジュールを更新する際に、スーパープロジェクトの記録された SHA-1 を使用する代わりに、サブモジュールのリモート追跡ブランチのステータスを使用します。使用されるリモートはブランチのリモート (branch.\<name>.remote) で、デフォルトは origin です。使用されるリモートブランチはデフォルトでリモート HEAD ですが、.gitmodules または .git/config の submodule.\<name>.branch オプションを設定することでブランチ名を上書きできます (.git/config が優先されます)。<br><br>これは、サポートされているすべての更新手順（--checkout、--rebaseなど）で機能します。変更されるのはターゲットのSHA-1のソースのみです。例えば、submodule update --remote --mergeは上流サブモジュールの変更をサブモジュールにマージし、submodule update --mergeはスーパープロジェクトのgitlinkの変更をサブモジュールにマージします。<br><br>最新の追跡ブランチの状態を保証するため、update --remote は SHA-1 を計算する前にサブモジュールのリモートリポジトリを取得します。取得したくない場合は、submodule update --remote --no-fetch を使用してください。<br><br>このオプションを使用すると、上流サブプロジェクトの変更をサブモジュールの現在のHEADに統合できます。あるいは、サブモジュールから git pull を実行することもできます。これはリモートブランチ名以外は同じです。update --remote はデフォルトの上流リポジトリと submodule.\<name>.branch を使用しますが、 git pull はサブモジュールの branch.\<name>.merge を使用します。デフォルトの上流ブランチをスーパープロジェクトと共に配布したい場合は submodule.\<name>.branch を、サブモジュール自体でよりネイティブな操作感を求める場合は branch.\<name>.merge を使用してください。
...
* --merge 
このオプションは update コマンドでのみ有効です。スーパープロジェクトに記録されたコミットをサブモジュールの現在のブランチにマージします。このオプションが指定された場合、サブモジュールの HEAD はデタッチされません。マージの失敗によりこの処理が実行できない場合は、サブモジュール内で発生した競合を通常の競合解決ツールで解決する必要があります。キー submodule.$name.update が merge に設定されている場合、このオプションは暗黙的に有効になります。

* --rebase 
このオプションはupdateコマンドでのみ有効です。現在のブランチをスーパープロジェクトに記録されたコミットにリベースします。このオプションが指定された場合、サブモジュールのHEADはデタッチされません。マージの失敗によりこの処理ができない場合は、git-rebase を使用してこれらの失敗を解決する必要があります。キーsubmodule.$name.updateがrebaseに設定されている場合、このオプションは暗黙的に有効になります。

* --init  
このオプションは更新コマンドでのみ有効です。更新前に、これまで「git submodule init」が呼び出されていないすべてのサブモジュールを初期化します。

* --name
このオプションはaddコマンドでのみ有効です。サブモジュールの名前を、デフォルトのパスではなく、指定された文字列に設定します。名前はディレクトリ名として有効なものでなければならず、/で終わってはなりません。
...
* --dissociate
このオプションはaddコマンドとupdateコマンドでのみ有効です。これらのコマンドはリモートリポジトリのクローンを必要とする場合があります。その場合、このオプションはgit-clone コマンドに渡されます。<br><br>注: --reference オプションについては、注を参照してください。
...
* \<path>…​
サブモジュールへのパス。これを指定すると、コマンドは指定されたパスにあるサブモジュールのみを操作するように制限されます。（この引数はaddコマンドでは必須です）。

## FILES
サブモジュールを初期化する際、各サブモジュールのURLを取得するために、サブモジュールを含むリポジトリの最上位ディレクトリにある.gitmodulesファイルが使用されます。このファイルは$GIT_DIR/configと同じ形式で記述する必要があります。各サブモジュールのURLのキーは「submodule.$name.url」です。詳細はgitmodules を参照してください。
