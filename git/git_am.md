...
本文の先頭にある「From:」「Date:」「Subject:」行は、ヘッダーから取得したコミット作成者名とタイトルの値を上書きします。

コミットメッセージは、「件名:」から取得したタイトル、空白行、そしてパッチの開始位置までのメッセージ本文で構成されます。各行末尾の余分な空白は自動的に削除されます。

パッチはメッセージの直後にインラインで記述する必要があります。以下の形式の行はすべて次のようになります。

* 3つのダッシュと行末、または
* 「diff -」で始まる行、または
* 「索引:」で始まる行

はパッチの開始とみなされ、コミットログメッセージは、そのような行が最初に出現する前に終了します。

git am を最初に起動する際に、処理対象のメールボックス名を指定します。適用できないパッチが最初に見つかると、処理は途中で中断されます。この状態から復旧するには、次の 2 つの方法があります。

1. --skip オプションを付けてコマンドを再実行することで、現在のパッチをスキップできます。
2. 作業ディレクトリ内の競合を手動で解決し、インデックスファイルを更新して、パッチ適用後に生成されるはずの状態に戻します。その後、--continue オプションを付けてコマンドを実行します。

このコマンドは、現在の操作が完了するまで新しいメールボックスの処理を拒否します。そのため、最初からやり直す場合は、メールボックス名を指定してコマンドを実行する前に、git am --abort を実行してください。

パッチを適用する前に、ORIG_HEAD は現在のブランチの先端に設定されます。これは、git am を間違ったブランチで実行した場合や、メールボックスを変更することで簡単に修正できるコミットのエラー (たとえば、「From:」行のエラー) など、複数のコミットに関する問題が発生した場合に役立ちます。

## HOOKS
このコマンドは、applypatch-msg、pre-applypatch、およびpost-applypatchフックを実行できます。詳細については、githooks[5]を参照してください。

## CONFIGURATION
このセクションのこの行より下の内容はすべて、git-config[1]のドキュメントから選択的に引用したものです。内容はそこに記載されているものと同じです。

* am.keepcr
これが真の場合、git-amはmbox形式のパッチに対して、--keep-crパラメータを付けてgit-mailsplitを呼び出します。この場合、git-mailsplitは\r\nで終わる行から\rを除去しません。コマンドラインから--no-keep-crを指定することで、この動作を上書きできます。git-amおよびgit-mailsplitを参照してください。

* am.threeWay
デフォルトでは、パッチが正常に適用できない場合、`git am` は失敗します。この設定を `true` にすると、パッチが適用対象の blob の識別情報を記録しており、かつそれらの blob がローカルに存在する場合、`git am` は 3-way マージによる適用を試みるようになります（コマンドラインで `--3way` オプションを指定するのと同等です）。デフォルト値は `false` です。`git-am` を参照してください。
