...
* -A \<num>
* --after-context \<num>
\<num> 行の末尾を表示し、連続する一致グループの間に -- を含む行を配置します。
...
* -W
* --function-context
関数名を含む前の行から次の関数名の前までの周囲のテキストを表示します。これにより、一致が見つかった関数全体が実質的に表示されます。関数名は、git diffがパッチのハンクヘッダーを決定するのと同じ方法で決定されます（gitattributes のカスタムハンクヘッダーの定義を参照）。

* -m \<num>
* --max-count \<num>
ファイルごとの一致数を制限します。-v または --invert-match オプションを使用する場合、指定された数の不一致が検出されると検索が停止します。値が -1 の場合、無制限に結果が返されます（デフォルト）。値が 0 の場合、0 以外のステータスで直ちに終了します。

* --threads \<num>
使用するgrepワーカースレッドの数。詳細については、「スレッドに関する注意事項」および「CONFIGURATION」の「grep.threads」を参照してください。

* -f \<file>
\<file> からパターンを 1 行ずつ読み取ります。<br><br>\<file> 経由でパターンを渡すと、\0 を含む検索パターンを提供できます。<br><br>すべてのパターンタイプが \0 を含むパターンをサポートしているわけではありません。指定されたパターンタイプがそのようなパターンをサポートできない場合、Git はエラーを出力します。PCRE v2 バックエンドでコンパイルされた --perl-regexp パターンタイプは、この種のパターンを最も広くサポートします。<br><br>Git 2.23.0 より前のバージョンでは、\0 を含むパターンは暗黙的に修正済みとみなされていました。これは文書化されておらず、例えば \0 を含む非 ASCII パターンと --ignore-case の間には、文書化されていない奇妙な相互作用も発生していました。<br><br>将来のバージョンでは、より多くの検索バックエンドで \0 を含むパターンをサポートするようになるかもしれませんが、それまでは、問題のパターン タイプがそれをサポートしていない場合は、私たちは死ぬことになります。

* -e
次のパラメータはパターンです。このオプションは - で始まるパターンに使用し、ユーザー入力をgrepに渡すスクリプトで使用します。複数のパターンはorで結合されます。

* --and
* --or
* --not
* ( …​ )
ブール式を使用して複数のパターンを組み合わせる方法を指定します。--or はデフォルトの演算子です。--and は --or よりも優先順位が高くなります。-e はすべてのパターンに使用する必要があります。

* --all-match
--or と組み合わせて複数のパターン式を指定する場合、このフラグを指定すると、それらすべてに一致する行を持つファイルに一致が制限されます。

* -q
* --quiet
一致した行を出力しません。代わりに、一致がある場合はステータス 0 で終了し、一致しない場合は 0 以外のステータスで終了します。

* \<tree>…​
作業ツリー内の追跡対象ファイルを検索する代わりに、指定されたツリー内の BLOB を検索します。

* --
オプションの終了を通知します。残りのパラメータは \<pathspec> の制限です。
...
* git grep time_t' -- *.[ch]
作業ディレクトリとそのサブディレクトリ内のすべての追跡された .c および .h ファイルで time_t を検索します。
...
--open-files-in-pager が使用されている場合、--threads オプション (および grep.threads 構成) は無視され、シングルスレッド実行が強制されます。<br><br>オブジェクトストアをgrepする際（--cachedオプションまたはツリーオブジェクトを指定した場合）、--textconvオプションが指定されていてテキスト変換が多すぎる場合、マルチスレッドで実行するとシングルスレッドよりもパフォーマンスが低下する可能性があります。したがって、このような場合にパフォーマンスが低下する場合は、--threads=1を使用することをお勧めします。

## CONFIGURATION
このセクションのこの行より下の部分は、git-config のドキュメントから選択的に引用したものです。内容はそこに記載されたものと同じです。
...
* grep.threads
使用する grep ワーカースレッドの数。設定されていない場合（または 0 に設定されている場合）、Git は利用可能な論理コアの数と同じ数のスレッドを使用します。

* grep.fullName
true に設定すると、--full-name オプションがデフォルトで有効になります。

* grep.fallbackToNoIndex
true に設定すると、git grep が git リポジトリ外で実行された場合に git grep --no-index にフォールバックします。デフォルトは false です。
