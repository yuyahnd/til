...
* -f \<file>
\<file> からパターンを 1 行ずつ読み取ります。<br><br>\<file> 経由でパターンを渡すと、\0 を含む検索パターンを提供できます。<br><br>すべてのパターンタイプが \0 を含むパターンをサポートしているわけではありません。指定されたパターンタイプがそのようなパターンをサポートできない場合、Git はエラーを出力します。PCRE v2 バックエンドでコンパイルされた --perl-regexp パターンタイプは、この種のパターンを最も広くサポートします。
