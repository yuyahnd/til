...
* -f \<file>
\<file> からパターンを 1 行ずつ読み取ります。<br><br>\<file> 経由でパターンを渡すと、\0 を含む検索パターンを提供できます。<br><br>すべてのパターンタイプが \0 を含むパターンをサポートしているわけではありません。指定されたパターンタイプがそのようなパターンをサポートできない場合、Git はエラーを出力します。PCRE v2 バックエンドでコンパイルされた --perl-regexp パターンタイプは、この種のパターンを最も広くサポートします。<br><br>Git 2.23.0 より前のバージョンでは、\0 を含むパターンは暗黙的に修正済みとみなされていました。これは文書化されておらず、例えば \0 を含む非 ASCII パターンと --ignore-case の間には、文書化されていない奇妙な相互作用も発生していました。
