...
exit $status
```
これにより、各テストの実行前にホットフィックス ブランチからの変更が適用されます。たとえば、ビルドまたはテスト環境が変更され、新しいリビジョンで既に適用されている修正が古いリビジョンで必要になる場合があります。(ホットフィックス ブランチが、バイセクトするすべてのリビジョンに含まれるコミットに基づいていることを確認してください。そうすることで、マージによって多くの変更が取り込まれなくなります。または、git merge ではなく git cherry-pick を使用してください。)
