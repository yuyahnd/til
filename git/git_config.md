...
|-e<br>--edit|指定された構成ファイルを変更するためのエディターを開きます。 --system、-global、またはrepository（デフォルト）のいずれか。|
|--[no-]includes|値を検索するときは、構成ファイルのinclude。*ディレクティブを尊重してください。 特定のファイルが指定されている場合（たとえば、-file、-globalなどを使用）はデフォルトでオフになり、すべての構成ファイルを検索する場合はオンになります。|
|--default \<value\>|--getを使用していて、要求された変数が見つからない場合、\<value\>がその変数に割り当てられた値であるかのように動作します。|
