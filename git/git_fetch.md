...
|\<group>|構成ファイルの remotes.\<group> の値としてリポジトリのリストを参照する名前。|
|\<refspec>|取得する参照と更新するローカル参照を指定します。 コマンド ラインに \<refspec> が表示されない場合、フェッチする参照は、代わりに remote.\<repository>.fetch 変数から読み取られます (以下の構成されたリモート トラッキング ブランチを参照してください)。|
|--stdin|引数として提供されたものに加えて、stdin から refspec を 1 行に 1 つずつ読み取ります。 「タグ <名前>」形式はサポートされていません。|
